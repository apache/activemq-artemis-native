...
public final class AioRing {
   ...
   public AioRing(long aioRingAddress) {
      ...
      final long nrAddress = aioRingAddress + NR_OFFSET;
      ...
      // this isn't going to change so can be cached on init
      this.nr = UNSAFE.getInt(nrAddress);
   }
   ...
   public int poll(long completedIoEvents, int min, int max) {
      ...
      final long startRingAioEvents = ioEventsAddress + (head * SIZE_OF_IO_EVENT_STRUCT);
      head += available;
      ...
      if (head < nr) {
         ...
         UNSAFE.copyMemory(ioEventsAddress, completedIoEvents, headerBytes);
         // copy header
      }
